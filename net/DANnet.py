...
class DNANet(nn.Module):
    def __init__(self, num_classes, input_channels, block, num_blocks, nb_filter,deep_supervision=False, get_feature=False):
    ...
    def forward(self, input):
        ...
        if self.deep_supervision and self.get_feature:
            output1 = F.sigmoid(self.final1(x0_1))
            output2 = F.sigmoid(self.final2(x0_2))
            output3 = F.sigmoid(self.final3(x0_3))
            output4 = F.sigmoid(self.final4(Final_x0_4))
            return [output1, output2, output3, output4], [x0_4, x1_3, x2_2, x3_1, x4_0]
...
class DNANet_withloss(nn.Module):
    ...
    def forward(self, img, label, curr_epoch_ratio, logit_label=None):
        ...
        if self.grad_pred is not None:
            grad_res = self.grad_pred(feature)
            grad_loss = self.mse_loss_fn(grad_res[label>0.1], logit_label[label>0.1])
            loss = loss * 0.9 + grad_loss * 0.1
        pred =res[-1]
        ...
        #     max_val = torch.max(res[-1][i, 0]).item()
        #     axes[i, 0].imshow(img[i,0].cpu().detach().numpy(), cmap='gray', vmin=0., vmax=1.)
        #     axes[i, 1].imshow(res[0][i,0].cpu().detach().numpy(), cmap='gray', vmin=0., vmax=1.)
        #     axes[i, 2].imshow(res[1][i,0].cpu().detach().numpy(), cmap='gray', vmin=0., vmax=1.)
        #     axes[i, 3].imshow(res[2][i,0].cpu().detach().numpy(), cmap='gray', vmin=0., vmax=1.)
        #     axes[i, 4].imshow(res[3][i,0].cpu().detach().numpy(), cmap='gray', vmin=0., vmax=1.)
        #     axes[i, 4].set_title('max:%.2f'%max_val)
