...
def parse_args():
    ...
    parser.add_argument("--valset", type=int, default=0, help="split trainset into trainset and valset")
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.valset == 0:
            val_indices, train_indices = split_indices_by_mod(0, len(trainset)-1, 4, 0)
            # 划分
    ...
    def training(self):
        ...
        for epoch in range(args.epochs):
            ...
            # for i, (data, label) in enumerate(self.train_data_loader):
                ...
                pred, softiouloss = self.net(data, label_b, label, epoch/args.epochs)
    ...
    def validation(self):
        ...
        base_log = "Data: {:s}, mIoU: {:.4f}/{:.4f}, prec: {:.4f}/{:.4f}, recall: {:.4f}/{:.4f}, F1: {:.4f}/{:.4f}, Pd:{:.4f}/{:.4f}, Fa:{:.8f}/{:.8f} "
        ...
        self.logger.info(
            base_log.format(self.args.dataset, miou_all, self.best_miou, prec_all,\
                             self.best_prec, recall_all, self.best_recall, fmeasure_all, self.best_fmeasure, PD,\
                                self.best_PD, FA, self.best_FA)
        )
